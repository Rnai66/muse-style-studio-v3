        ...
        {image ? (
          <div className="preview-wrap">
            ...
            <div className="preview-actions">
              <button className="pa-btn" onClick={() => takePhoto(CameraSource.Camera)}>📷 ถ่ายใหม่</button>
              <button className="pa-btn" onClick={clear}>✕ ลบ</button>
          ...
          <div className="upload-empty" onClick={() => takePhoto(CameraSource.Photos)}>
            {camLoading
              ...
              : <>
                  <div className="upload-silhouette">👤</div>
                  <p className="upload-label">แตะเพื่ออัปโหลดรูป</p>
                  <p className="upload-hint">หรือถ่ายรูปใหม่เพื่อลองชุด</p>
                  <div className="upload-btns">
                    <button className="up-btn" onClick={(e) => { e.stopPropagation(); takePhoto(CameraSource.Photos); }}>🖼 คลังรูป</button>
                    <button className="up-btn" onClick={(e) => { e.stopPropagation(); takePhoto(CameraSource.Camera); }}>📷 กล้อง</button>
                  </div>
                </>
            }
          </div>
        ...
        <div className="chat-messages" ref={chatRef}>
          {messages.map((m, i) => (
            <div key={i} className={`bubble-wrap ${m.role}`}>
              {m.role === 'assistant' && <div className="ai-label">MUSE ✦</div>}
              <div className={`bubble ${m.role}`}>{m.content}</div>
            </div>
          ))}
          {aiLoading && (
            <div className="bubble-wrap assistant">
              <div className="ai-label">MUSE ✦</div>
